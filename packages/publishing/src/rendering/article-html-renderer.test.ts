...
test("legacy escaped quote markup is repaired before channel HTML is generated", () => {
  const rendered = renderArticleDocumentHtml(
    contentPackage([
      {
        id: "paragraph",
        type: "paragraph",
  ...
  equal(rendered.html, '<p>共同主题是<strong>"更便宜"</strong>以及<strong>"更可靠"</strong>。</p>');
});
